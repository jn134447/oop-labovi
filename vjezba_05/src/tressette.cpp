...
Deck::Deck()
{
    std::array<Suit, 4> suits = {Suit::Hearts, Suit::Diamonds, Suit::Clubs, Suit::Spades};
...
void Deck::deal_cards(std::vector<Player> &players)
{
    ...
    for (Player &player : players)
    {
        ...
        std::copy(this->cards.begin(), this->cards.begin() + HAND_SIZE, player.hand.begin());
        this->cards.erase(this->cards.begin(), this->cards.begin() + HAND_SIZE);

        // WARN: REMOVE DEBUG HANDS
        player.hand = {{1, Suit::Clubs},  {1, Suit::Hearts}, {1, Suit::Diamonds}, {1, Suit::Spades},
                       {2, Suit::Clubs},  {2, Suit::Hearts}, {2, Suit::Diamonds}, {2, Suit::Clubs},
...
{
    auto by_card_numbers = [](Card &card_1, Card &card_2) { return card_1.number < card_2.number; };
...
unsigned short Player::multi_of_kind_points()
{
    ...
    {
        int count = std::count_if(hand.begin(), hand.end(), [&number](const Card &c) { return c.number == number; });
        if (count == 4)
...
Tressette::Tressette(Mode mode)
{
    ...
    case Mode::oneVersusOne:
        players.reserve(TWO_PLAYERS);
        players.emplace_back("player_one");
        players.emplace_back("player_two");
        break;
    case Mode::twoVersusTwo:
        players.reserve(FOUR_PLAYERS);
        players.emplace_back("player_one");
        players.emplace_back("player_two");
        players.emplace_back("player_three");
        players.emplace_back("player_four");
        break;
    }
}

void Tressette::print_mode()
{
    std::cout << "mode: ";
    switch (mode)
    {
...
void Tressette::print_players_info()
{
    for (Player &player : players)
    {
...
void Tressette::print_deck_contents()
{
...
void Tressette::print_game_state()
{
    print_mode();
    std::cout << std::endl;
